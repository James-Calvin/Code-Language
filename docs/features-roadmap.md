...
| `[x]` | Bytecode/VM | Header v0x09 + debug table | Implemented |
...
| `[x]` | Runtime/stdlib | Math helpers, constants, and randomness | Implemented on `vm-native` and `vm-web`: `minimum`, `maximum`, `absolute`, `sign`, `lerp`, `sine`, `cosine`, `random`, plus built-in constants `pi` and `tau` |
...
| `[x]` | Platform/targets | Static-site web build workflow | Implemented: public `.code` input emits a runnable folder named after the entry file (or custom `-o` / `--output`) with `index.html`, embedded bytecode, copied `assets/` when present, and optional `app.bytecode` via `--emit-web-bytecode` |
