...
| `[~]` | Platform/targets | Browser-backed web app runtime | Implemented current slice: a dedicated worker owns the bytecode-v10 VM, fixed/continuous update scheduling, `MainScene` lifecycle, and draw command encoding; the main thread owns resize, Canvas replay, keyboard/primary pointer input, asset-backed audio, visibility, and `requestAnimationFrame`; generated Blob-worker apps run directly from `file://`; expand advanced input/content handling and fuller audio mixing |
