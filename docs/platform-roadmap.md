...
| `[~]` | Phase 4 | Browser-backed web app runtime | Implemented current slice: worker-hosted Rust/Wasm bytecode-v10 VM and lifecycle execution, fixed-60 default or explicit cooperative-continuous scheduling, transferable Canvas command buffers, display-synchronized/capped rendering, centered `640x360` safe area, browser-backed input/audio, diagnostics, profiling, and direct `file://` startup |
