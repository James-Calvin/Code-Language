...
- objects, records, and interfaces with compile-time generic type parameters and individual/grouped inline interface implementations; static fields/methods with same-type bare static lookup; record key selection (`key` / `ignore key`); arrays, maps, sets, queues, stacks, optionals as `T` or `none`, and typed recoverable `fallible` errors
