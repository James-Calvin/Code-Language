...
| `[~]` | Platform/targets | Host ABI baseline | Implemented baseline `HOST_CALL` opcode + native binding table (`std.io.print`, `std.time.*`); expand to engine capabilities and web host |
