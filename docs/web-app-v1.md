...
- Maintainer builds may select `--web-backend direct-wasm`, which additionally emits `code-app.wasm` with an embedded direct-file fallback. This typed backend executes lifecycle/application functions natively in Wasm and is now the primary compiler/runtime focus, but the public default does not switch until browser compatibility, memory behavior, bytecode-free host metadata, and complete error/fuzz parity pass.
- Direct-Wasm diagnostic builds may pass `--disable-garbage-collection`. The flag is rejected for other backends, records disabled-GC metadata in generated apps, and is allowed to leak memory.
- Browser compatibility validation is centralized in `node scripts/test-browser-compat.mjs`. It runs generated direct-Wasm apps in local Chromium-family desktop browsers and writes a mobile/manual report page for iOS and Android browser checks.
