...
| `[x]` | Object model | Interfaces + conformance checks | Compile-time field/method interface declarations, explicit `implement Interface for Type { ... }` conformance, empty implement blocks for data-only interfaces, method mappings with signature/return checks, and inline object-body syntax `implement Interface.method(...) { ... }` |
