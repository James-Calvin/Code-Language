...
- Generated apps run bytecode, lifecycle functions, and update scheduling in a dedicated JavaScript worker. The worker source is embedded and launched through a Blob URL, so a generated `index.html` still runs directly from `file://`.
...
- The generated runtime is a bytecode-v10 JavaScript VM with pooled metadata, predecoded instructions, contiguous locals frames, and slot-backed object fields. The main-thread VM remains a reference/test path. Wasm remains gated on parity and a measured 2x improvement over the worker JavaScript baseline.
