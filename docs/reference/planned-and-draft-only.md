...
| Broader target-agnostic expansion of the graphical app profile | First web-entry slice is implemented with top-level lifecycle authoring and usage-based implied engine imports across web-app modules; carrying that authoring shape cleanly into future native graphical targets remains planned. |
