...
- Expressions: arithmetic (including `%`), comparisons, logical and/or/not (short-circuit), assignment (including `+=`, `-=`, `*=`, `/=`, `%=` and postfix `++/--` across variables, object fields, and array elements), function calls, integer literals with decimal/binary/octal/hex prefixes, full-expression string interpolation/concat, and escaped literal interpolation braces with `\{` / `\}`.
...
## Not Implemented (yet)
- Language gaps: fallible-error propagation shorthand after the explicit `on error` model is exercised.
