...
- `code-runtime.wasm`: required Rust bytecode-v10 VM for generated applications
...
- Generated apps run the Rust/Wasm bytecode VM, lifecycle functions, and update scheduling in a dedicated worker. The worker source is embedded and launched through a Blob URL, so a generated `index.html` still runs directly from `file://`.
...
- Generated apps enable the opt-in VM profiler with `?code-profile=1`; worker-backed `CodeRuntime.profile.start()`, `stop()`, `reset()`, `report()`, and `json()` return promises. `await CodeRuntime.profile.report()` prints instruction, function, host-call, allocation, and stack metrics.
...
- The generated runtime is a bytecode-v10 Rust/Wasm VM with predecoded instructions, contiguous stacks, 16-byte tagged values, handle-backed collections, slot-backed fields, and tracing garbage collection. The JavaScript VM remains a reference/conformance path.
