...
- Expressions: arithmetic (including `%`), comparisons, logical and/or/not (short-circuit), assignment (including `+=`, `-=`, `*=`, `/=`, `%=` and postfix `++/--` across variables, object fields, and array elements), function calls, integer literals with decimal/binary/octal/hex prefixes, decimal real literals (`1.5`, `1.`, `.5`), explicit numeric casts, enum-to-integer casts, integer-to-enum casts with literal validation, full-expression string interpolation/concat, and escaped literal interpolation braces with `\{` / `\}`.
...
## Not Implemented (yet)
- Language gaps: fallible-error propagation shorthand after the explicit `on error` model is exercised; sized numerics, numeric literal suffixes, exponent notation, semicolon injection, `fallible<void, E>`, and `foreach` over non-array collections. Planned map `foreach` should yield entry values.
