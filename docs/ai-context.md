...
- Interfaces: `interface Name { function<...> method(...); }` plus explicit `implement Interface for Object { method(types...) via Object.method; }` conformance checks (compile-time only).
...
## Not Implemented (yet)
- User-defined data remaining: records, visibility enforcement, interface-typed values/calls and dynamic dispatch.
