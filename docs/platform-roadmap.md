...
- Current recommended authoring models:
  - small apps may use the inferred web entry profile with top-level `start` / `update` / `draw` / optional `drawHud` and usage-based implied engine imports (`Draw` / `Input` / `Viewport` / `Colors` / `Diagnostics` / `Runtime` / `Audio`, plus direct `Color` and canonical `engine.scene` types)
...
- Generated apps now use the gated Rust/Wasm bytecode-v12 VM in the existing worker. The JavaScript VM remains the reference/conformance path; browser scheduling and Canvas/audio/input ownership remain JavaScript responsibilities.
...
- `vm-web`: static site output with `index.html`, embedded bytecode/direct-file Wasm fallback, and a cacheable `code-runtime.wasm`. `app.bytecode` is emitted only when `--emit-web-bytecode` is passed.
...
| `[~]` | Phase 4 | Browser-backed web app runtime | Implemented current slice: worker-hosted Rust/Wasm bytecode-v12 VM and lifecycle execution, fixed-60 default or explicit cooperative-continuous scheduling, source-aware runtime diagnostics, transferable Canvas command buffers, display-synchronized/capped rendering, centered `640x360` safe area, browser-backed input/audio, diagnostics, profiling, and direct `file://` startup |
...
3. **Browser-backed app runtime**
   - Implemented current slice: generated app page and browser runtime fill the window; a worker-hosted Rust/Wasm VM owns bytecode state and lifecycle execution while JavaScript schedules fixed/continuous updates and the main thread replays Canvas commands and owns input/audio/visibility.
   - Performance follow-up: measure generated-scene capacity and host-call/Canvas costs; pursue direct Code-to-Wasm only if Wasm interpretation remains dominant.
   - Product next step: expand advanced input/content handling and fuller audio mixing while keeping the higher-level engine-facing API off raw window handles.

4. **Engine packages + loop contract**
   - Implemented current slice: importable wrapper modules now exist for colors, drawing, input, viewport queries, diagnostics, runtime scheduling, audio, scene composition, and scene-loop execution under `lib/engine/`.
