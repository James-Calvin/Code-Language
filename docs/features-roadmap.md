...
| `[~]` | Compiler pipeline | Typed direct-Wasm backend | Typed semantic model, structured native control flow, dependency-free C# Wasm emitter, core language parity, worker integration, and benchmark gate are implemented behind `--web-backend direct-wasm`; tracing reclamation and bytecode-free host metadata still block default adoption |
