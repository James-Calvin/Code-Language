...
- Notes-derived app direction: add a target-agnostic graphical app profile that can synthesize the entry shell from top-level lifecycle authoring and an implicit engine prelude for `--build-web` first, while keeping explicit `MainScene` valid and preserving a path to future native graphical targets.
...
- Expressions: arithmetic (including `%` and truncating integral `/`), comparisons, logical and/or/not (short-circuit), assignment (including `+=`, `-=`, `*=`, `/=`, `%=` and postfix `++`/`--` across variables, object fields, and array elements), function calls, integer literals with decimal/binary/octal/hex prefixes, decimal real literals (`1.5`, `1.`, `.5`), explicit numeric casts, enum-to-integer casts, integer-to-enum casts with literal validation, full-expression string interpolation/concat, and escaped literal interpolation braces with `\{` / `\}`. Use a `real` operand such as `1. / 2` or `1 as real / 2` for real division.
...
- Web app/runtime slice: `--build-web` emits a static site folder (`index.html` + `app.bytecode`, plus copied `assets/` content when present) with a generated full-window browser runtime, centered `640x360` safe area, hybrid-expanded visible world, `MainScene` metadata, and `start/update/draw` plus optional `draw_hud` driving the JS VM. Current web builds also embed bytecode in `index.html` for direct opening; planned polish is embed-only by default with a debug/inspection flag for writing `app.bytecode`.
...
## Not Implemented (yet)
- Language gaps: field defaults such as `integer radius = 7;`; fallible-error propagation shorthand after the explicit `on error` model is exercised; sized numerics with `byte` as the readable alias for `whole8`; numeric literal suffixes, exponent notation, semicolon injection, `fallible<void, E>`, and `foreach` over non-array collections. Planned map `foreach` should yield entry values.
- Stdlib/runtime gaps: broader standard-library surface beyond the current math/random and core container baseline.
- Engine/app ergonomics gaps: target-agnostic graphical app profile with top-level lifecycle authoring and implicit engine prelude; byte-channel `rgb`/`rgba` overloads after `byte` exists; keep explicit `MainScene` valid.
...
- Engine web runtime maturation: broaden the current `lib/engine/` wrapper layer beyond colors/drawing/input/viewport, add richer input/audio/content handling, prevent page scroll/panning from app keys, route normal web-app `print` output to the browser console by default, and replace the remaining raw window-handle `engine.window`/`engine.input`/`engine.gfx` stubs with real browser-backed behavior or wrappers.
...
- 2026-04-13: Captured notes-derived gap decisions in docs/roadmaps: field defaults are planned, sized numerics should include `byte` as the readable alias for `whole8`, byte-channel color overloads wait for `byte`, a target-agnostic graphical app profile should reduce `MainScene`/import boilerplate while preserving explicit `MainScene`, generated web apps should prevent page scroll/panning and route normal `print` output to the browser console, and web builds should move toward embed-only bytecode by default with a debug/inspection `app.bytecode` option.
