...
| `[x]` | Object model | Builder ergonomics | Implemented: primary constructors, implicit record field-order constructors, implicit `TypeName(...)` construction, and default parameters for functions/methods/constructors |
...
| `[x]` | Object model | Static members | Implemented for object/record static fields, static constants, static methods, external `TypeName.member` access, and same-type bare static lookup; static interface requirements and static constructors remain deferred |
...
| `[x]` | Object model | Interfaces + conformance checks | Compile-time field/method contracts, external `implement Interface for Type`, individual inline `implement Interface.method(...)`, and complete grouped inline `implement Interface { function... }` blocks with generic specialization, overload, completeness, and conflict validation |
| `[~]` | Object model | Interface dispatch lowering/runtime model | Baseline runtime dispatch table opcode implemented for interface-typed locals/params/returns/fields/arrays, with interface field get/set support; extend to broader container types and optimizer-grade fast paths |
...
| `[~]` | Compiler pipeline | Typed direct-Wasm backend | Typed semantic model, structured native control flow, dependency-free C# Wasm emitter, core language parity, worker integration, benchmark gate, direct-Wasm browser compatibility suite, and diagnostic `--disable-garbage-collection` flag are implemented behind `--web-backend direct-wasm`; browser compatibility, memory behavior, bytecode-free host metadata, and full error/fuzz parity still block default adoption |
